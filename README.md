...
- 주차장 모형: 1.2m × 1.22m 아크릴, 24구역 (6×3 그리드), 차량 주행로 반영
...
- 배선도, Z축 도면
- 프로젝트 소개 영상 (1분 30초, 무음·자막 전용)

### 🎬 소개 영상 구성

심사위원 편의를 위해 **나레이션·효과음 없이 자막만으로** 구성했습니다. 영상은 팀 강점 3가지 — ①전류 피드백 기반 미세조정(정밀 정렬), ②실물 레일 ↔ Unity 디지털 트윈 1:1 동기화, ③앱 입력→스케줄링→자동 이동의 완결된 파이프라인 — 를 뼈대로 삼아 전체 시간의 60% 이상을 강점 시연에 배정했습니다.

전체 흐름은 **차량 정보·출차 시간 등록 → 우선순위 산출 → 레일 이동(배속) → Z축 상승 → 충전 시작 → 미세조정으로 최대 전력 탐색 → 실시간 그래프로 피크 확인** 순으로 이어집니다.

| 씬 | 구간 | 내용 |
|----|------|------|
| S1 훅 | 0:00–0:08 | 충전구역 불법주정차 신고 급증(3년 만에 6.7배) 제시 → "충전기가 차를 찾아간다면?" |
| S2 앱 입력 | 0:08–0:18 | 구역(A-3)·차량번호·배터리(16%)·출차시간 입력 후 충전 신청 |
| S3 스케줄링 | 0:18–0:28 | 출차 시간·거리·배터리 종합해 방금 등록한 차량이 1순위로 산출되는 과정 |
| S4 레일 이동 ★ | 0:28–0:45 | 좌우 분할 화면으로 실물 레일과 Unity 트윈이 동시에 같은 궤적 이동(×4 배속 표기) |
| S5 Z축·충전 | 0:45–0:55 | 정속 클로즈업으로 Z축 상승 후 수신 코일 접근, 송신부 전원 인가(LED 점등) |
| S6 미세조정 ★ | 0:55–1:13 | 상하 분할 — 코일 미세 스캔과 실시간 전력 그래프가 피크에서 최대치로 고정 (영상 하이라이트) |
| S7 모니터링 | 1:13–1:22 | Unity 대시보드에 3D 주차장·스케줄·전력 그래프를 한 화면에서 관제, 피크 시간대 분산 |
| S8 클로징 | 1:22–1:30 | 전 과정 타임랩스(×8 배속) 후 로고 카드 |

> 배속 구간은 좌상단에 배속 표기를 노출해 **사실을 숨기지 않는 편집**으로 신뢰를 확보하고, 소리가 없는 대신 숫자 강조·색상 변화·화살표 모션으로 흐름을 전달합니다.
